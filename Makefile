...
APPNAME ="NIX"
APP_LOAD_PARAMS += --path $(APP_PATH)
else ifeq ($(COIN),gincoin)
# GINcoin
DEFINES   += COIN_P2PKH_VERSION=38 COIN_P2SH_VERSION=10 COIN_FAMILY=1 COIN_COINID=\"DarkCoin\" COIN_COINID_HEADER=\"GINCOIN\" COIN_COLOR_HDR=0x398DD8 COIN_COLOR_DB=0x97BCDE COIN_COINID_NAME=\"GINcoin\" COIN_COINID_SHORT=\"GIN\" COIN_KIND=COIN_KIND_GINCOIN
APPNAME ="GINcoin"
APP_LOAD_PARAMS += --path $(APP_PATH)
else
ifeq ($(filter clean,$(MAKECMDGOALS)),)
$(error Unsupported COIN - use bitcoin_testnet, bitcoin, bitcoin_cash, bitcoin_gold, litecoin, dogecoin, dash, zcash, horizen, komodo, stratis, peercoin, pivx, viacoin, vertcoin, stealth, digibyte, qtum, bitcoin_private, zcoin, gamecredits, zclassic, xsn, nix, gincoin)
endif
endif
...
listvariants:
	@echo VARIANTS COIN bitcoin_testnet bitcoin bitcoin_cash bitcoin_gold litecoin dogecoin dash zcash horizen komodo stratis peercoin pivx viacoin vertcoin stealth digibyte qtum bitcoin_private zcoin gamecredits zclassic xsn nix gincoin
